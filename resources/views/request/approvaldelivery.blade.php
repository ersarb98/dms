@extends('layouts.app')
@section('pageTitle', 'Approval Order Delivery')

@section('content')
    <div class="container">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5>Approval Order Delivery</h5>
            </div>
            <div class="card-body">
                    ...
                    <tbody>
                        @foreach ($orders as $order)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('deliveries.show', $order->id) }}" class="btn btn-info btn-sm">
                                        View Details
                                    </a>
                                    <button class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#confirmModal"
                                        data-action="{{ route('approvaldel.approve', $order->id) }}"
                                        data-type="approve">Approve</button>
                                    <button class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#confirmModal"
                                        data-action="{{ route('approvaldel.reject', $order->id) }}"
